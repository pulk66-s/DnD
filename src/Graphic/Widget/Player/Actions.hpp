...
namespace dnd::graphic::widget::wplayer
{
    ...
    private:
        ...
        std::unordered_map<player::PlayerMoney, int *> spendMoney = {
            {player::PlayerMoney::PC, new int(0)},
            {player::PlayerMoney::PA, new int(0)},
            {player::PlayerMoney::PE, new int(0)},
            {player::PlayerMoney::PO, new int(0)},
            {player::PlayerMoney::PP, new int(0)}
        };
        ...
        void displayAttack() {
            ...
            if (ImGui::BeginMenu(("Weapon " + (this->weapon == nullptr ? "" : this->weapon->name())).c_str())) {
                for (objects::items::Weapon *weapon : this->player->weapons()) {
        ...
        void displayTakeDamages() {
            ...
            if (ImGui::Button("Take")) {
                this->player->hp(-*this->takeDamage);
            }
            ImGui::End();
        }
        void displaySpendMoney() {
            ...
            for (std::pair<player::PlayerMoney, int *> p : this->spendMoney) {
                ImGui::InputInt(player::toString(p.first).c_str(), p.second);
            }
            ...
            if (*this->spendMoneyConfirmation) {
                if (ImGui::Begin("Spend money confirmation", this->spendMoneyConfirmation)) {
                    ...
                    for (std::pair<player::PlayerMoney, int *> p : this->spendMoney) {
                        ImGui::Text("%s: %d", player::toString(p.first).c_str(), *p.second);
                    }
                    if (ImGui::Button("Yes")) {
                        std::unordered_map<player::PlayerMoney, int> c = this->player->coins();
                        for (std::pair<player::PlayerMoney, int *> p : this->spendMoney) {
